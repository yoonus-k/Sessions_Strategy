//+------------------------------------------------------------------+
//|                                  SessionsStrategyV2_Enhanced.mq5  |
//|  Second EA alongside SessionsStrategyV2.mq5 (spec D1), same        |
//|  Include/*.mqh modules, zero shared-module edits - every           |
//|  difference below is an input default or a small additive gate     |
//|  living entirely in this file. CONSOLIDATED 2026-09-15 after a     |
//|  6-variant search (Enhanced/2/3/4/5/6, see IMPLEMENTATION_PLAN.md  |
//|  build log for the full history) into the single best-tested       |
//|  config - this file IS what was called "Enhanced5" during that     |
//|  search, renamed here as the sole survivor. Deep-analysis          |
//|  findings that motivated each change, and what got REJECTED along |
//|  the way, so a future pass doesn't re-try a dead end blind:        |
//|                                                                    |
//|  KEPT (each individually verified live, not just post-hoc CSV):   |
//|   1. InpEntryMode = EM_REVERSAL_ONLY (was EM_BOTH). REV trades     |
//|      averaged +0.039R/PF 1.10 vs BOS's -0.069R/PF 0.85 in the      |
//|      full unfiltered 2020-2026 sample - BOS was a net drag.        |
//|   2. InpEntryWindowMinutes = 15 (was 30). Edge decayed across the  |
//|      window: first 5min avg +0.052R vs the 15-20min bucket's       |
//|      -0.162R in the unfiltered sample.                             |
//|   3. InpMinAnchorScore = 0.90 (new gate, 0=disabled elsewhere).    |
//|      Sub-0.90-scoring anchor sessions averaged PF 0.75, clearly    |
//|      the worst quality bucket. Enforced in CheckAnchorPreRoll():   |
//|      a too-low-scoring anchor is treated exactly like "not found"  |
//|      (reuses the existing AVWAP-suppression path - no new state).  |
//|   4. InpBlockContinuationShort = true (new gate). CONTINUATION+    |
//|      SHORT was the single worst regime/direction cell (avg         |
//|      -0.073R) - shorting "with the trend" at session open during   |
//|      gold's 2020-2026 structural uptrend (~1500 -> ~4400).         |
//|      Enforced inline at the trade-opening gate.                    |
//|   5. InpMaxSpreadPoints = 12 (was 50). Tightest-spread quintile    |
//|      (<=5pts) averaged PF 1.74 vs PF~0.83 for 7-16pts, live.       |
//|   6. InpMaxAnchorLookbackHr = 24 (was 48). Anchors 24-48h old were |
//|      the clear worst bucket (PF 0.51) live; 0-24h all >=1.06.     |
//|   7. InpBeTriggerR = 0.5 (unchanged) - EMPIRICALLY VALIDATED, not  |
//|      a leftover guess: both 0.35 and 0.75 were tried live and both |
//|      came back worse (avg +0.044R and +0.033R vs 0.5's +0.065R in  |
//|      the pre-(5)/(6) baseline) - 0.5 sits at or near a local       |
//|      optimum for this entry population.                            |
//|                                                                     |
//|  REJECTED (tested live, made things worse - do not re-add):        |
//|   - InpRevConfirmCloses = 2 (was 1): tested alone, came back        |
//|     roughly FLAT (avg -0.007R, PF 0.98) - worse than not filtering |
//|     at all. The idea was to filter the "moves in favor immediately |
//|     then reverses" fakeout shape by waiting one more confirming    |
//|     close; in practice it just delays entries into a worse part    |
//|     of the move without adding real selectivity. Stacked together  |
//|     with changes 5+6 above (that combo was called "Enhanced2"),    |
//|     it dragged an otherwise-excellent result (avg +0.125R/PF 1.33  |
//|     for 5+6 alone) down to +0.080R/PF 1.20 AND introduced a        |
//|     3-straight-losing-year pattern (2024/25/26) that changes 5+6   |
//|     alone do not have (2025 flips positive, 2024 remains the one   |
//|     unresolved open question - negative in every variant tested).  |
//|                                                                    |
//|  Live result with the KEPT set above (2020-2026, n=281): win rate  |
//|  37.7%, avg +0.125R/trade, PF 1.33, positive in 6 of 7 years       |
//|  (only 2024 negative, small n=23 - real regime issue or variance,  |
//|  not yet determined).                                              |
//|                                                                    |
//|  Original file header follows unmodified for context:              |
//|                                                                    |
...
input long             InpMagic  = 20260915; // different from the main EA (20260914) so both can run side by side without colliding
...
input int    InpEntryWindowMinutes  = 15;    // ENHANCED default (was 30): edge decayed across the window in the 2020-2026 sample - no NEW trade after this many minutes since session open; 0 = unlimited
...
input double InpMaxAnchorLookbackHr = 24.0; // ENHANCED default (was 48): anchors 24-48h old were the clear worst bucket, PF 0.51
input double InpAnchorFlexMin       = 120.0;
input double InpAnchorPreRollMin    = 30.0;
input double InpMinAnchorScore      = 0.90; // ENHANCED (new gate, 0=off): anchor source session's quality score must clear this or the session trades with no anchor (same as "not found") - sub-0.90 was the clearly-worst quality bucket in the 2020-2026 sample
...
input group "Entry"
input ENUM_ENTRY_MODE_V2 InpEntryMode     = EM_REVERSAL_ONLY; // ENHANCED default: REV clearly beat BOS in the 2020-2026 sample (avg +0.039R vs -0.069R)
...
input group "Enhanced filters (2026-09-15, new in this fork only)"
input bool InpBlockContinuationShort = true; // don't take a SHORT while the session's regime is CONTINUATION (locked-Z1) - the single worst regime/direction cell in the 2020-2026 sample (avg -0.073R), shorting with a multi-year structural uptrend at session open
...
input int  InpMaxSpreadPoints      = 12; // ENHANCED default (was 50): tightest-spread quintile averaged PF 1.74 vs PF~0.83 for 7-16pts
...
input string InpCsvPrefix      = "SessionsStrategyV2_Enhanced_Trades"; // different filename so this never overwrites the main EA's export
   ...
   for(int k=0;k<2;k++)
     {
      ...
      bool belowMinScore=(ar.found && InpMinAnchorScore>0.0 && ar.srcScore<InpMinAnchorScore);
      if(belowMinScore) ar.found=false; // ENHANCED gate: too-low-quality anchor treated as "not found"
      ...
      if(InpVerbose)
        {
         if(ar.found)
            ...
            PrintFormat("[V2] pre-roll %.0fmin before %s %s -> ANCHOR SCORE %.2f < InpMinAnchorScore %.2f -> SKIP",
                        InpAnchorPreRollMin,V2_SessionName(s),RiyDate(realOpen),ar.srcScore,InpMinAnchorScore);
         else
...
void RefreshDash()
  {
   ...
   g_dash.Title(0,StringFormat("--- SessionsStrategyV2 ENHANCED %s ---",InpExecute?"LIVE":"DRY RUN (I1)"));
   ...
   if(currRole==ROLE_TRADE)
     {
      ...
      //--- ENHANCED gate: CONTINUATION regime (locked at session open, ZONE_Z1) + SHORT was
      //--- the single worst regime/direction cell in the 2020-2026 sample (avg -0.073R) -
      //--- shorting with the trend at session open during a multi-year structural uptrend.
      bool blockedContShort=(InpBlockContinuationShort && dec.take && dec.regimeLocked &&
                             dec.regimeZone==ZONE_Z1 && dec.direction==DIR_SHORT);
      if(blockedContShort && InpVerbose)
         PrintFormat("[V2] TAKE SHORT blocked: CONTINUATION regime + SHORT (InpBlockContinuationShort)");

      if(dec.take && g_tm.CanOpenNew() && g_time.WithinEntryWindow(now) && !blockedContShort)
        {
